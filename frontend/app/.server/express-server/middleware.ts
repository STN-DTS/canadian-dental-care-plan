...
/**
 * Creates named request-logging middleware with production-aware Morgan format.
 *
 * Health readiness requests are excluded from request logging.
 */
export function createRequestLoggingMiddleware(isProduction: boolean): RequestHandler {
  const log = createLogger('express/middleware/requestLoggingMiddleware');
  ...
  return function requestLoggingMiddleware(request, response, next) {
    ...
    return morganMiddleware(logFormat, { stream: { write: (msg) => log.http(msg.trim()) } });
  };
}
